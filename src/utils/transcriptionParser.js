 ...
 * It also handles optional numbering (e.g., 1. TRANSCRIÇÃO DE ÁUDIO:) and missing brackets.
 *
...
export const extractAudioTranscription = (text) => {
  ...
  // Match the start tag: [TRANSCRIÇÃO DE ÁUDIO] (brackets, colon and leading number optional)
  // \s* matches any whitespace between components
  const startPattern = /(?:\d+\.\s*)?\[?TRANSCRIÇÃO DE ÁUDIO\]?\s*:?/i;
  ...
  // Supporting optional brackets, colon, numbering and spaces
  const endPattern = /(?:\d+\.\s*)?\[?(?:TEXTO IDENTIFICADO(?: NAS IMAGENS)?|DESCRIÇÃO VISUAL)\]?\s*:?/i;
