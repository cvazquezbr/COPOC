...
export default async function handler(req) {
  try {
    ...
    // Security check: Ensure the URL is a Vercel Blob Storage URL
    ...
    // Fetch the file from the Vercel Blob Storage
    const response = await fetch(urlToProxy);

    // Check if the request to the blob storage was successful
    if (!response.ok) {
