...
export default function apiMiddleware(req, res, next) {
  ...
  if (url.pathname.startsWith('/api/')) {
    ...
    const apiRoute = url.pathname.substring(5);
    const apiFilePath = join(process.cwd(), 'api', `${apiRoute}.js`);

    if (existsSync(apiFilePath)) {
      const apiModulePath = 'file://' + apiFilePath;

      import(apiModulePath).then(async (module) => {
        try {
